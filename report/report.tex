...
\begin{itemize}[leftmargin=1.2cm]
  ...
  \item $Q$ belongs to a specified format (short-answer, MCQ, fill-in-the-blank, reasoning, or comparison),
...
The standard evaluation datasets used in this project---SQuAD v2 \citep{rajpurkar2016squad}, RACE \citep{lai2017race}, and HotpotQA \citep{yang2018hotpotqa}---represent three complementary difficulty levels. SQuAD contains extractive factoid questions from Wikipedia passages. RACE comprises multiple-choice questions from English-language examinations across a wide range of topics. HotpotQA requires multi-hop reasoning across two documents, making it the most challenging of the three.
...
Tokenization converts cleaned text to memory-mapped binary arrays using the \texttt{tiktoken r50k\_base} encoding \citep{radford2019language}. The base vocabulary has 50,257 tokens; three special-purpose tokens are appended: \texttt{<|im\_start|>} (ID 50257), \texttt{<|im\_end|>} (ID 50258), and \texttt{<|pad|>} (ID 50259), yielding a final vocabulary of 50,260 tokens.
...
The SFT dataset is constructed from two reading-comprehension sources, each converted to a shared ChatML instruction format:
\\
\\
...
\begin{itemize}[leftmargin=1.2cm]
  ...
  \item \textbf{Early stopping heuristics}: stops generation on \texttt{<|im\_end|>}, on a question-mark for non-MCQ types, or when all four MCQ options appear.
\end{itemize}
...
Figures~\ref{fig:demo_short1}--\ref{fig:demo_complex} show representative inference outputs from the deployed model for two short-answer inputs and one complex input.
...
\begin{table}[H]
  ...
  \begin{tabular}{L{6.5cm} L{7.0cm}}
    ...
    \texttt{src/data/build\_sft\_dataset.py} & ChatML SFT dataset construction from two sources \\
